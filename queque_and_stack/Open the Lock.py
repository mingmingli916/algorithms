...
class Solution(object):
    def openLock(self, deadends, target):
        ...
        while queue:
            step += 1
            # iterate the nodes which is already in the queue
            size = len(queue)
            for _ in range(size):
                cur = queue.popleft()
                if cur == target:
                ...
                for i in [-1, 1]:
                    for j in range(len(cur)):
                        neighbor = cur[:j] + str((int(cur[j]) + 10 + i) % 10) + cur[j + 1:]

                        # add node which has not been visited and is not in dead ends
